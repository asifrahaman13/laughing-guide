...
func (s *fileService) ProcessFile(organizationId string) (bool, error) {
	bucket := os.Getenv("AWS_BUCKET_NAME")
	fmt.Println(organizationId)

	csvContent, err := s.awsRepository.DownloadFileContent(bucket, fmt.Sprintf("%s.csv", organizationId))
	if err != nil {
...
func (s *fileService) GetSampleFile(key string) (map[string]string, error) {
	bucket := os.Getenv("AWS_BUCKET_NAME")
